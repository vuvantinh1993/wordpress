...
function type_scripts() {
	...
	// Theme stylesheet
	wp_enqueue_style( 'type-style', get_stylesheet_uri(), array(), '1.1.1' );

	// Blocks stylesheet.
	wp_enqueue_style( 'type-blocks-style', get_template_directory_uri() . '/inc/css/blocks.css', array( 'type-style' ), '20190115' );
 ...
 */
function type_block_editor_styles() {
	// Add Block styles.
	wp_enqueue_style( 'type-block-editor-style', get_theme_file_uri( '/inc/css/editor-blocks.css' ) );
	// Add Google fonts.
	wp_enqueue_style( 'type-fonts', type_fonts_url(), array(), null );
	// Add editor styles.
	wp_add_inline_style( 'type-block-editor-style', type_custom_style() );
}
add_action( 'enqueue_block_editor_assets', 'type_block_editor_styles' );
